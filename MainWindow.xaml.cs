...
namespace TPL
{
	public partial class MainWindow : Window
	{
		...
		private readonly List<ObjectId> tempManualSelectionIds = new();
		private readonly List<DBObject> transientObjects = new();
		...
		private PlotHelper.PlotSettingsData BuildCurrentSettings()
		{
			...
			{
				DeviceName = cbPrinters.SelectedItem?.ToString(),
				PaperSize = cbPapers.SelectedItem?.ToString(),
				PlotStyle = cbStyles.SelectedItem?.ToString() ?? "",
				OutputPath = txtPath.Text,
				FrameType = rbBlockMode.IsChecked == true ? PlotHelper.FrameType.Block : PlotHelper.FrameType.Polyline
			};
