...
namespace TPL
{
	...
	public static class PlotLogic
	{
		...
		public static void PlotAll(List<PlotFrame> frames, PlotHelper.PlotSettingsData settings)
		{
			...
			var progressForm = new ProgressWindow(L10n.T("prog_title"), plotJobs.Count);
			Application.ShowModelessWindow(Application.MainWindow.Handle, progressForm);
			...
			try
			{
				...
				for (int i = 0; i < plotJobs.Count; i++)
				{
					...
					progressForm.UpdateProgress(i + 1, string.Format(L10n.T("prog_progress"), i + 1, plotJobs.Count), string.Format(L10n.T("prog_file"), fileName));
				...
				progressForm.UpdateProgress(plotJobs.Count, string.Format(L10n.T("prog_progress"), plotJobs.Count, plotJobs.Count), "");

				// Phase 2: Post-processing Orientation
				if (settings.Orientation != PlotHelper.PlotOrientation.Auto && generatedFiles.Count > 0)
				{
					progressForm.SetSubTitle("Applying Orientation...");
					...
					&& generatedFiles.All(f => f.EndsWith(".pdf", StringComparison.OrdinalIgnoreCase)))
				{
					try
					{
						progressForm.SetMax(generatedFiles.Count);
						progressForm.SetSubTitle(L10n.T("prog_merging"));
						...
						using (var outDoc = new PdfDocument())
						{
							...
							foreach (string f in generatedFiles)
							{
								...
								mergedCount++;
								progressForm.UpdateProgress(mergedCount, string.Format(L10n.T("prog_progress"), mergedCount, generatedFiles.Count), L10n.T("prog_merging"));
							}
				...
				else if (settings.ConvertToImage && generatedFiles.All(f => f.EndsWith(".pdf", StringComparison.OrdinalIgnoreCase)))
				{
					...
					{
						progressForm.SetMax(generatedFiles.Count);
						progressForm.SetSubTitle("Converting to Image...");
						...
						for (int i = 0; i < generatedFiles.Count; i++)
						{
							...
							progressForm.UpdateProgress(i + 1, string.Format(L10n.T("prog_progress"), i + 1, generatedFiles.Count), "Converting to Image...");
						}
